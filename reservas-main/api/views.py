...
from .utils import users_in_group, notify_users  # ← NUEVO
import csv
...
class ReservationCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        resources = list(Resource.objects.filter(id__in=resource_ids).values_list("name", flat=True))
...
class ReservationDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["approval"] = Approval.objects.filter(reservation=self.object).first()
        return ctx
...
def my_history(request):
    """
    Historial del usuario. Enriquecemos cada reserva con:
      - r.appr (Approval o None)
      - r.can_cancel (bool) usando MIN_CANCEL_WINDOW_HOURS
    """
    qs = Reservation.objects.filter(user=request.user).order_by("-start").select_related("space")

    approvals_map = Approval.objects.filter(reservation__in=qs).in_bulk(field_name="reservation_id")
    now = timezone.now()
    min_hours = getattr(settings, "MIN_CANCEL_WINDOW_HOURS", 2)

    for r in qs:
        setattr(r, "appr", approvals_map.get(r.id))
        can_time = (r.start - now).total_seconds() >= min_hours * 3600
        setattr(r, "can_cancel", r.status in [Reservation.PENDING, Reservation.APPROVED] and can_time)
...
def cancel_reservation(request, pk):
    ...
    Notifica al grupo de Aseo si estaba pendiente/aprobada y aún no inicia.
    """
    if request.method != "POST":
    ...
    # guardamos estado previo para lógica de aviso a Aseo
    prev_status = reservation.status

    reservation.cancel_by_user(reason=reason, actor=request.user)
    messages.success(request, "Reserva cancelada correctamente.")

    # Notificar a Aseo si estaba programado algo operacionalmente relevante
    if prev_status in (Reservation.PENDING, Reservation.APPROVED) and reservation.start > timezone.now():
        cleaning_users = users_in_group(settings.CLEANING_GROUP_NAME)
        msg = (
            f"Reserva CANCELADA: {reservation.space.name} "
            f"({reservation.start:%Y-%m-%d %H:%M} → {reservation.end:%H:%M}). "
            f"Motivo usuario: {reason or '—'}"
        )
        email_subject = "Reservas Inacap · Reserva CANCELADA"
        email_body = (
            f"Se canceló una reserva:\n\n"
            f"- Espacio: {reservation.space.name}\n"
            f"- Inicio:  {reservation.start:%Y-%m-%d %H:%M}\n"
            f"- Término: {reservation.end:%Y-%m-%d %H:%M}\n"
            f"- Usuario: {reservation.user.username}\n"
            f"- Motivo usuario: {reason or '—'}\n"
        )
        notify_users(cleaning_users, msg, email_subject, email_body)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if decision == "APPR":
                ...
                # Aviso a Aseo (detalle operativo)
                cleaning_users = users_in_group(settings.CLEANING_GROUP_NAME)
                resources_txt = ""
                if reservation.purpose:
                    # extrae una línea de recursos si usamos el patrón "Recursos solicitados:"
                    for part in reservation.purpose.split("|"):
                        part = part.strip()
                        if part.lower().startswith("recursos solicitados"):
                            resources_txt = " " + part
                            break

                msg = (
                    f"Reserva APROBADA: preparar {reservation.space.name} "
                    f"({reservation.start:%Y-%m-%d %H:%M} → {reservation.end:%H:%M})."
                    f"{resources_txt and ' ' + resources_txt}"
                )
                email_subject = "Reservas Inacap · Reserva APROBADA (preparación)"
                email_body = (
                    f"Se aprobó una reserva:\n\n"
                    f"- Espacio: {reservation.space.name}\n"
                    f"- Inicio:  {reservation.start:%Y-%m-%d %H:%M}\n"
                    f"- Término: {reservation.end:%Y-%m-%d %H:%M}\n"
                    f"- Usuario: {reservation.user.username}\n"
                    f"- Notas admin: {notes or '—'}\n"
                    f"- Observación del solicitante: {reservation.purpose or '—'}\n"
                )
                notify_users(cleaning_users, msg, email_subject, email_body)
...
def export_reservations_csv(request):
    ...
    writer.writerow(["ID","Usuario","Espacio","Inicio","Fin","Estado","Motivo de solicitud","Notas aprobación"])
